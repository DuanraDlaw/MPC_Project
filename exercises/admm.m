...
%% run admm
    ...
    for k=1:MAX_ITER
       ...
       %%************************ Your code here ************************%%
        s = [-zeros(n*(N+1)+m*N,1) + rho*(bar_z - mu); b];
        % Using Matlab solver at each step
% 		Z = S\s;
        % Using precomputed Matlab inverse
        Z = inv_S*s;
