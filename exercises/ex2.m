...
while kappa > opt.epsilon % Stop once the barrier term is small
  ...
  while 1
    ...
    sum1 = 0;
    sum2 = 0;
    for i=1:length(prob.G),
        sum1 = sum1 + 1/(prob.d(i)-prob.G(i,:)*z)*prob.G(i,:)';
        sum2 = sum2 + 1/((prob.d(i)-prob.G(i,:)*z)^2)*prob.G(i,:)'*prob.G(i,:);
    end
    
    hessian = prob.H + kappa*sum2;
    Dz = hessian\(-prob.H*z - prob.q - kappa*sum1 );
